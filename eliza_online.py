...
Downloaded from https://github.com/dhconnelly/paip-python on 5/5/14
Subsequent revisions by Richard Turner (rwturner@stanford.edu)

This should all be replaced with pattern matching based on the existing 
regex libraries.
...
def interact(prompt, rules, default_responses, logfile):
    ...
    logfile.write(thistime.now().strftime('%c') + '\n')
    while input != 'FAIL' and input != 'THANKS': #useful to have stop conditions for success and failure
        try:
        ...
        logfile.write(prompt + response + '\n')
        print response
    logfile.write(50*'-'+'\n')
    logfile.close()
    
def interact_online(input, rules, default_responses):
    """Have a conversation with a user."""
...
def respond(rules, input, default_responses):
    ...
    # Look through rules and find the first input pattern that matches the input.
    matching_rules = []
    ...
    # When rules are found, choose one of its responses at random.
